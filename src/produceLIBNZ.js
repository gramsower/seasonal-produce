export class VegetablesNZ {
  constructor (vegName, vegSeason =[], vegInfo) {
    ...
    this.VegArrNZ = [nopal, onion, olive, radish, tomato];
  }
}
...
let olive = new VegetablesNZ('olive', '', 'Olives come in a variety of colors and may come whole or with the pit already removed. Fresh, cured olives should be kept in the refrigerator to extend their life for a few weeks.');
let radish = new VegetablesNZ('radish', '', 'Most radishes are sold with the tops attached: look for vibrant greens and firm roots. Do not forget you can saute the tops or use them in pesto!');
...
// let VegArrNZ = [nopal, onion, olive, radish, tomato];
...
export class LegumesNZ {
  constructor (legumeName, legumeSeason =[], legumeInfo) {
    ...
    this.legumeArrNZ = [shellBeans, peas, snapPeas, peanuts, snowPeas];
  }
}
let shellBeans = new LegumesNZ('shell beans', '', 'Fresh shell beans cook much quicker than their dried out counterparts. Look for pods that have pronounced, evenly sized bumps and avoid those with noticeable empty spots.');
...
let snapPeas = new LegumesNZ('snap peas', '', 'Snap peas are excellent raw or cooked. When shopping, look for those with full pods that are vibrant in color, avoiding any with mushy spots.');
...
let snowPeas = new LegumesNZ('snow peas', '', 'Good snow peas will be bright green with thin, flexible pods. Snow peas are mildly flavored and can be served raw or cooked.');
